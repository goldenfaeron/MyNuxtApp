					...
					<v-card :color="item.color">
						<v-card-title primary-title>
							<div>
								...
								<p>
									Rating:
									<v-icon v-for="(rating, index) in item.rating" :key="index">mdi-star</v-icon>
									<v-icon v-for="(nonrated, index) in 5-item.rating" :key="index">mdi-star-outline</v-icon>
								</p>
							</div>
						</v-card-title>
						<v-img aspect-ratio="1" max-height="500" :src="webRoot + item.image.path"></v-img>
						<v-card-actions>
							<v-btn color="white" @click="addToCart(item)" text>Buy</v-btn>
						</v-card-actions>
					</v-card>
				</v-flex>
			</v-layout>
